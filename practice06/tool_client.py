    ...
    try:
        ...
        try:
            ...
            if "error" in response_data:
                return json.dumps({"status": "error", "message": response_data["error"]}, ensure_ascii=False)
...
def load_skill_content(skill_name):
    try:
        ...
        if not skill_dir:
            return json.dumps({"status": "error", "message": f"未找到技能: {skill_name}"}, ensure_ascii=False)
...
def get_skills_json_for_system_prompt():
    result = list_available_skills()
    result_data = json.loads(result)

    if result_data.get('status') == 'success':
        skills = result_data.get('data', [])
...
def main():
    ...
    skills_json = get_skills_json_for_system_prompt()
    ...
    tools = [
        ...
        },
        {
            ...
            "function": {
                ...
                "description": "获取所有可用的技能列表，包括每个技能的名称和描述",
                "parameters": {
                    "type": "object",
                    "properties": {},
                    "required": {}
        ...
        },
        {
            ...
            "function": {
                ...
                "description": "加载指定技能的具体内容，当需要使用某个技能时调用此函数获取技能的完整说明",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "skill_name": {
    ...
    system_prompt = f"""你是一个具有文件操作、网络访问、聊天历史搜索和技能调用能力的AI助手，能够执行以下操作：
...
10. 加载技能内容：使用 load_skill_content 工具加载指定技能的具体内容
...
当用户请求涉及撰写、修改或润色通知时，请首先使用 list_available_skills 查看可用技能，如果发现有相关的skill（如notice技能），请立即使用 load_skill_content 加载该技能的内容，并严格按照技能说明执行。

技能使用流程：
1. 用户提出需求 → 判断是否需要使用技能
2. 如果需要 → 使用 list_available_skills 获取技能列表
3. 如果有匹配的技能 → 使用 load_skill_content 加载技能内容
4. 严格按照技能说明执行用户请求
...
当使用 anythingllm_query 工具访问文档仓库后，你可以根据返回的结果向用户提供相关信息。"""
    ...
    print("4. 访问文档仓库：查询文档仓库中的内容")
    print("5. 使用技能：请撰写一个关于五一节放假的通知")
    print()
                    ...
                    if message.get('tool_calls'):
                        for tool_call in message.get('tool_calls', []):
                            ...
                            if tool_name == "list_available_skills":
                                ...
                                if result_data.get('status') == 'success':
                                    skills = result_data.get('data', [])
                                    updated_system = chat_history[0]["content"]
                                    new_skills_json = json.dumps({"skills": skills}, ensure_ascii=False, indent=2)
                                    updated_system = updated_system.replace(
                                        f"你具有以下技能可以使用：\n{skills_json}",
                                        f"你具有以下技能可以使用：\n{new_skills_json}"
                                    )
                                    chat_history[0]["content"] = updated_system
                                ...
                                result_data = json.loads(tool_result)
                                if result_data.get('status') == 'success':
                                    skill_content = result_data.get('data', '')
                                    chat_history.append(message)
                                    chat_history.append({
                                        "role": "tool",
                                        "tool_call_id": tool_call.get('id'),
                                    ...
                                    print("助手: ", end='', flush=True)
                                    skill_response = stream_llm(chat_history)
                                    if skill_response:
                                        chat_history.append({"role": "assistant", "content": skill_response})
                                    continue
